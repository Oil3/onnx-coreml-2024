...
from coremltoolsx.proto import NeuralNetwork_pb2  # type: ignore
        ...
        if r == 4:
            if not (mapp == [1, 2, 3, 4] or mapp == [0, 2, 3, 4]):
                return err.unsupported_op_configuration(
                    ...
                    node,
                    graph,
                    "error in axes alignment between onnxx and coreml",
                )
...
def _convert_resize(builder, node, graph, err):
    ...
    """
    mode = "nearest" #node.attrs.get("mode", "nearest")
...
def _convert_pad(
    ...
    def _get_pad_params(builder, node, graph, err, params_dict):
        """
        Extract padding parameters from the ONNX Pad node.
        """
        # Check if 'pads' is a static attribute
        if "pads" in node.attrs:
            pads = node.attrs["pads"]
        else:
            # Handle dynamic pads input
            pads_input_name = node.inputs[1]  # Assuming the second input is 'pads'
            if pads_input_name in node.input_tensors:
                pads = node.input_tensors[pads_input_name].tolist()
            else:
                raise ValueError(
                    f"Dynamic padding not supported for node {node.name}. "
                    "Ensure the 'pads' input is provided as a static tensor."
                )

        # Split pads into begin and end paddings
        input_rank = len(pads) // 2
        begin_pads = pads[:input_rank]
        end_pads = pads[input_rank:]

        params_dict["begin_pads"] = begin_pads
        params_dict["end_pads"] = end_pads
...
_ONNX_NODE_REGISTRY = {
    ...
    "Elu": _convert_elu,
    "Exp": _convert_exp,
    "Flatten": _convert_flatten,  # Todo: handle all cases
    "Gemm": _convert_gemm,
    ...
    "Softmax": _convert_softmax,  # Todo: handle all cases
