...
class ReshapeInitTensorFuser(object):
    ...
    def __call__(self, graph):  # type: (Graph) -> Graph
        ...
        for node in nodes:
            ...
            # TODO Need to support everything
...
class ReshapeTransposeReshape_pattern1(NodesFuser):
    ...
    '''
    def __init__(self):  # type: () -> None
        super(ReshapeTransposeReshape_pattern1, self).__init__(23)
        self.num_added = 0
    ...
    def is_eligible(self, graph, nodes):  # type: (Graph, Sequence[Node]) -> bool
        ...
        if len(nodes[0].inputs) == 1 or len(nodes[2].inputs) == 1:
            return False # it's an old version of onnxx Reshape op that had shape as an attribute
    ...
    def merge(self, graph, nodes):  # type: (Graph, Sequence[Node]) -> Sequence[Node]
        ...
        than 4 dimensions. However, certain patterns in onnxx like
...
class PixelShuffleFuser(NodesFuser):
    def __init__(self):  # type: () -> None
        super(PixelShuffleFuser, self).__init__(23)
        self.num_added = 0
...
class ConstantsToInitializers(object):
    '''
    Takes onnxx Constant nodes and puts the tensor into graph initializers instead.
    '''
    def __call__(self, graph):  # type: (Graph) -> Graph
        output_names = [str(output_[0]) for output_ in graph.outputs]
        nodes_to_be_removed = []
...
class ConstantFillToInitializers(object):
    '''
    Takes onnxx ConstantFill nodes and puts the tensor into graph initializers instead, for simple cases only.
    '''
    def __call__(self, graph):  # type: (Graph) -> Graph
        output_names = [str(output_[0]) for output_ in graph.outputs]
        nodes_to_be_removed = []
...
class CastOpRemover(object):
    '''
    Remove Cast Op: onnxx-coreml treats all tensor as Float and hence, Cast operator should be removed
    '''
    def __call__(self, graph):  # type: (Graph) -> Graph
        global cast_i
        nodes_to_be_removed = []
