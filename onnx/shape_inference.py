...
"""onnxx shape inference. Shape inference is not guaranteed to be
complete.
...
import onnxx
import onnxx.onnx_cpp2py_export.shape_inference as C  # noqa: N812
from onnxx import AttributeProto, FunctionProto, ModelProto, TypeProto
...
def infer_shapes(
    ...
    if isinstance(model, (ModelProto, bytes)):
        ...
        return onnxx.load_from_string(inferred_model_str)
...
def infer_node_outputs(
    schema: onnxx.defs.OpSchema,
    node: onnxx.NodeProto,
    input_types: dict[str, onnxx.TypeProto],
    input_data: dict[str, onnxx.TensorProto] | None = None,
    input_sparse_data: dict[str, onnxx.SparseTensorProto] | None = None,
    opset_imports: list[onnxx.OperatorSetIdProto] | None = None,
    ir_version: int = onnxx.IR_VERSION,
) -> dict[str, onnxx.TypeProto]:
    ...
    return {key: onnxx.TypeProto.FromString(out) for key, out in outputs.items()}
