...
from onnxx import ONNX_ML, ModelProto, NodeProto, TypeProto, ValueInfoProto, numpy_helper
from onnxx.backend.base import Backend
from onnxx.backend.test.case.test_case import TestCase
from onnxx.backend.test.loader import load_model_tests
from onnxx.backend.test.runner.item import TestItem
...
class Runner:
    ...
    def download_model(
        ...
        try:
            ...
            print("Done")
            onnxx.utils._extract_model_safe(download_file.name, models_dir)
    ...
    def prepare_model_data(cls, model_test: TestCase) -> str:
        onnx_home = os.path.expanduser(
            os.getenv("ONNX_HOME", os.path.join("~", ".onnxx"))
        )
        ...
        if not os.path.exists(os.path.join(model_dir, "model.onnxx")):
    ...
    def _add_model_test(self, model_test: TestCase, kind: str) -> None:
        ...
        def run(test_self: Any, device: str, **kwargs) -> None:  # noqa: ARG001
            if model_test.url is not None and model_test.url.startswith(
                ...
                model_pb_path = os.path.join(model_dir, "model.onnxx")
                use_dummy = False

            if not ONNX_ML and "ai_onnx_ml" in model_dir:
                return

            model = onnxx.load(model_pb_path)
            ...
            if use_dummy:
                ...
                # model stored in onnxx/backend/test/data/light,
                ...
                for i in range(len(onx.graph.input)):
                    ...
                    with open(name, "wb") as f:
                        f.write(onnxx.numpy_helper.from_array(value).SerializeToString())
                ...
                if expected_outputs is None:
                    ref = onnxx.reference.ReferenceEvaluator(onx)
                    outputs = ref.run(None, feeds)
                    for i, o in enumerate(outputs):
                        name = os.path.join(test_data_set, f"output_{i}.pb")
                        with open(name, "wb") as f:
                            f.write(onnxx.numpy_helper.from_array(o).SerializeToString())
                else:
