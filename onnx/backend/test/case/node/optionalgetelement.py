...
class OptionalHasElement(Base):
    ...
    def export_get_element_tensor() -> None:
        ...
        tensor_type_proto = onnxx.helper.make_tensor_type_proto(
            elem_type=onnxx.TensorProto.FLOAT,
            shape=[
                4,
            ],
        )
        optional_type_proto = onnxx.helper.make_optional_type_proto(tensor_type_proto)
    ...
    def export_get_element_sequence() -> None:
        ...
        tensor_type_proto = onnxx.helper.make_tensor_type_proto(
            elem_type=onnxx.TensorProto.INT32,
            shape=[
                4,
            ],
        )
        seq_type_proto = onnxx.helper.make_sequence_type_proto(tensor_type_proto)
        optional_type_proto = onnxx.helper.make_optional_type_proto(seq_type_proto)
