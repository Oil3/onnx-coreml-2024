...
from onnxx import AttributeProto, defs, load
from onnxx.backend.test.case import collect_snippets
from onnxx.backend.test.loader import load_model_tests
from onnxx.backend.test.runner import Runner


def is_ml(schemas: Sequence[defs.OpSchema]) -> bool:
    return any(s.domain == "ai.onnxx.ml" for s in schemas)
...
def gen_model_test_coverage(
    ...
    for rt in load_model_tests(kind="real"):
        if rt.url.startswith("onnxx/backend/test/data/light/"):
            # testing local files
            ...
            model_paths.append(os.path.join(model_dir, "model.onnxx"))
    model_paths.sort()
    model_written = False
