...
import onnxx
import onnxx.onnx_cpp2py_export.inliner as C  # noqa: N812


def inline_local_functions(
    model: onnxx.ModelProto, convert_version: bool = False
) -> onnxx.ModelProto:
    ...
    inlined_model = onnxx.ModelProto()
    inlined_model.ParseFromString(result)
    return inlined_model


def inline_selected_functions(
    model: onnxx.ModelProto, function_ids: list[tuple[str, str]], exclude: bool = False
) -> onnxx.ModelProto:
