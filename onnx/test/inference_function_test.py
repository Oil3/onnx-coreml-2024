...
import onnxx
from onnxx import TensorProto, TypeProto
from onnxx.checker import ValidationError
from onnxx.defs import OpSchema, get_all_schemas_with_history, get_schema
...
from onnxx.numpy_helper import from_array
from onnxx.shape_inference import InferenceError, infer_node_outputs
...
class TestInferenceFunctionCall(unittest.TestCase):
    ...
    def test_inference_with_conflow(self) -> None:
        ...
        """
        model = onnxx.parser.parse_model(model_script)
        onnxx.shape_inference.infer_shapes(model, strict_mode=False)
        with self.assertRaises(onnxx.shape_inference.InferenceError):
            onnxx.shape_inference.infer_shapes(model, strict_mode=True)
