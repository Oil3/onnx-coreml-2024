...
import onnxx
import onnxx.helper as oh
import onnxx.numpy_helper as onh
import onnxx.reference as orf
...
def create_model():
    ...
    .. code-block:: python
        ...
        from onnxx importonnx.TensorProto
        from onnxx.helper import oh.make_tensor
        ...
        def loop_range_cond_only(A: FLOAT["N"]) -> FLOAT["N"]:
            T = A
            cond = op.Constant(value=make_tensor("true",onnxx.TensorProto.BOOL, [1], [1]))
            while cond:
                T = T + A
    ...
    sparse_initializers = []
    functions = []
    inputs.append(oh.make_tensor_value_info("A", onnxx.TensorProto.FLOAT, shape=("N",)))
    nodes.append(
        oh.make_node(
            "Constant",
            [],
    ...
    def _make_local_graph_body():
        ...
        sparse_initializers = []
        inputs.append(
            oh.make_tensor_value_info("infinite_loop", onnxx.TensorProto.INT64, shape=[])
        )
        inputs.append(
            oh.make_tensor_value_info("cond", onnxx.TensorProto.BOOL, shape=[])
        )
        inputs.append(oh.make_tensor_value_info("T", onnxx.TensorProto.UNDEFINED, []))
        ...
        outputs.append(
            oh.make_tensor_value_info("cond_out", onnxx.TensorProto.BOOL, shape=[])
        )
        outputs.append(oh.make_tensor_value_info("T_0", onnxx.TensorProto.UNDEFINED, []))
        graph = oh.make_graph(
            nodes,
            "loop_body",
            inputs,
    ...
    outputs.append(
        oh.make_tensor_value_info("T_2", onnxx.TensorProto.FLOAT, shape=("N",))
    )
    graph = oh.make_graph(
        nodes,
