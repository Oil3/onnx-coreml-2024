...
class TestSchema(unittest.TestCase):
    ...
    def test_attr_default_value(self) -> None:
        ...
        self.assertEqual(type(v), onnxx.AttributeProto)
        self.assertEqual(v.type, onnxx.AttributeProto.FLOAT)

    def test_function_body(self) -> None:
        self.assertEqual(
            type(defs.get_schema("Selu").function_body), onnxx.FunctionProto
        )
...
class TestAttribute(unittest.TestCase):
    ...
    def test_init_with_default_value(self):
        ...
        self.assertIsInstance(default_value, onnxx.AttributeProto)
...
class TestOpSchemaRegister(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.assertFalse(onnxx.defs.has(self.op_type, self.op_domain))

    def tearDown(self) -> None:
        # Clean up the registered schema
        for version in [*self.trap_op_version, self.op_version]:
            with contextlib.suppress(onnxx.defs.SchemaError):
                onnxx.defs.deregister_schema(self.op_type, version, self.op_domain)
    ...
    def test_register_multi_schema(self):
        for version in [*self.trap_op_version, self.op_version]:
            ...
            self.assertTrue(onnxx.defs.has(self.op_type, version, self.op_domain))
        for version in [*self.trap_op_version, self.op_version]:
            # Also make sure the `op_schema` is accessible after register
            registered_op = onnxx.defs.get_schema(
    ...
    def test_register_schema_raises_error_when_registering_a_schema_twice(self):
        ...
        onnxx.defs.register_schema(op_schema)
        with self.assertRaises(onnxx.defs.SchemaError):
            onnxx.defs.register_schema(op_schema)
    ...
    def test_deregister_the_specified_schema(self):
        for version in [*self.trap_op_version, self.op_version]:
            ...
            onnxx.defs.register_schema(op_schema)
            self.assertTrue(onnxx.defs.has(op_schema.name, version, op_schema.domain))
        onnxx.defs.deregister_schema(op_schema.name, self.op_version, op_schema.domain)
        for version in self.trap_op_version:
            self.assertTrue(onnxx.defs.has(op_schema.name, version, op_schema.domain))
        # Maybe has lesser op version in trap list
        if onnxx.defs.has(op_schema.name, self.op_version, op_schema.domain):
            schema = onnxx.defs.get_schema(
    ...
    def test_deregister_schema_raises_error_when_opschema_does_not_exist(self):
        with self.assertRaises(onnxx.defs.SchemaError):
            onnxx.defs.deregister_schema(self.op_type, self.op_version, self.op_domain)
    ...
    def test_legacy_schema_accessible_after_deregister(self):
        ...
        onnxx.defs.register_schema(op_schema)
        schema_a = onnxx.defs.get_schema(
            op_schema.name, op_schema.since_version, op_schema.domain
        )
        schema_b = onnxx.defs.get_schema(op_schema.name, op_schema.domain)

        def filter_schema(schemas):
            return [op for op in schemas if op.name == op_schema.name]

        schema_c = filter_schema(onnxx.defs.get_all_schemas())
        schema_d = filter_schema(onnxx.defs.get_all_schemas_with_history())
