...
import onnxx
from onnxx import TensorProto, helper
...
class TestAutomaticUpgrade(automatic_conversion_test_base.TestAutomaticConversion):
    ...
    def test_Loop_1(self) -> None:
        ...
        x_in = onnxx.helper.make_tensor_value_info("x_in", onnxx.TensorProto.FLOAT, [1])
        cond_out = onnxx.helper.make_tensor_value_info(
            "cond_out", onnxx.TensorProto.BOOL, []
        )
        x_out = onnxx.helper.make_tensor_value_info("x_out", onnxx.TensorProto.FLOAT, [1])
        x_scan = onnxx.helper.make_tensor_value_info(
            "x_scan", onnxx.TensorProto.FLOAT, [1]
        )
        const = onnxx.helper.make_node(
            "Constant",
            inputs=[],
            outputs=["one"],
        ...
        add = onnxx.helper.make_node("Add", inputs=["x_in", "one"], outputs=["x_out"])
        id_1 = onnxx.helper.make_node("Identity", inputs=["x_out"], outputs=["x_scan"])
        id_2 = onnxx.helper.make_node(
    ...
    def test_Loop_2(self) -> None:
        ...
        x_in = onnxx.helper.make_tensor_value_info(
            "x_in", onnxx.TensorProto.FLOAT, [2, 1]
        )
        cond_out = onnxx.helper.make_tensor_value_info(
            "cond_out", onnxx.TensorProto.BOOL, []
        )
        x_out = onnxx.helper.make_tensor_value_info(
            "x_out", onnxx.TensorProto.FLOAT, [2, 1]
        )
        squeeze = onnxx.helper.make_node(
            "Squeeze", inputs=["x_in"], outputs=["squeeze_out"], axes=[1]
        )
        unsqueeze = onnxx.helper.make_node(
            "Unsqueeze", inputs=["squeeze_out"], outputs=["x_out"], axes=[1]
        )
        identity = onnxx.helper.make_node(
            "Identity", inputs=["cond_in"], outputs=["cond_out"]
        )
        loop_body = onnxx.helper.make_graph(
    ...
    def test_Scan(self) -> None:
        ...
        next_in = onnxx.helper.make_tensor_value_info(
            "next_in", onnxx.TensorProto.FLOAT, [2]
        )
    ...
    def test_ops_tested(self) -> None:
        ...
        all_schemas = onnxx.defs.get_all_schemas()
