...
import onnxx
from onnxx import TensorProto, ValueInfoProto, helper, shape_inference, version_converter

LATEST_OPSET = onnxx.defs.onnx_opset_version()


class TestAutomaticConversion(unittest.TestCase):
    def _test_model_conversion(
        self, to_opset: int, model: str | onnxx.ModelProto
    ) -> None:
        if isinstance(model, str):
            model = onnxx.parser.parse_model(model)
        ...
        onnxx.checker.check_model(converted)
