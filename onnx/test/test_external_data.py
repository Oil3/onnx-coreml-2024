...
from onnxx.external_data_helper import (
...
from onnxx.numpy_helper import from_array, to_array
...
class TestLoadExternalDataBase(unittest.TestCase):
    ...
    def get_temp_model_filename(self) -> str:
        return os.path.join(self.temp_dir, str(uuid.uuid4()) + ".onnxx")
    ...
    def create_external_data_tensor(
        ...
        tensor.data_location = onnxx.TensorProto.EXTERNAL
        return tensor

    def create_test_model(self, location: str = "") -> str:
        constant_node = onnxx.helper.make_node(
            "Constant",
            inputs=[],
            outputs=["values"],
        ...
        inputs = [
            helper.make_tensor_value_info(
                "input_value", onnxx.TensorProto.FLOAT, self.initializer_value.shape
            )
        ]
        ...
        model_filename = os.path.join(self.temp_dir, "model.onnxx")
        onnxx.save_model(model, model_filename, self.serialization_format)
...
class TestLoadExternalData(TestLoadExternalDataBase):
    def test_load_external_data(self) -> None:
        model = onnxx.load_model(self.model_filename, self.serialization_format)
    ...
    def test_save_external_data(self) -> None:
        ...
        os.mkdir(temp_dir)
        new_model_filename = os.path.join(temp_dir, "model.onnxx")
        onnxx.save_model(model, new_model_filename, self.serialization_format)

        new_model = onnxx.load_model(new_model_filename, self.serialization_format)
...
class TestLoadExternalDataSingleFile(TestLoadExternalDataBase):
    ...
    def test_save_external_invalid_single_file_data_and_check(
        ...
        new_model_filepath = os.path.join(model_dir, "model.onnxx")
        ...
        onnxx.save_model(model, new_model_filepath, self.serialization_format)
        if use_model_path:
            with self.assertRaises(onnxx.checker.ValidationError):
                _ = onnxx.load_model(new_model_filepath, self.serialization_format)
        else:
            onnx_model = onnxx.load_model(
...
class TestSaveAllTensorsAsExternalData(unittest.TestCase):
    ...
    def test_convert_model_to_external_data_with_size_threshold(self) -> None:
        ...
        onnxx.save_model(self.model, model_file_path, self.serialization_format)

        model = onnxx.load_model(model_file_path, self.serialization_format)
        initializer_tensor = model.graph.initializer[0]
    ...
    def test_convert_model_to_external_data_from_one_file_with_location(self) -> None:
        ...
        model_file_path = self.get_temp_model_filename()
        onnxx.save_model(model, model_file_path, self.serialization_format)
        model = onnxx.load_model(model_file_path, self.serialization_format)
        initializer_tensor = model.graph.initializer[0]
...
class TestExternalDataToArray(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self._temp_dir_obj = tempfile.TemporaryDirectory()
        self.temp_dir: str = self._temp_dir_obj.name
        self._model_file_path: str = os.path.join(self.temp_dir, "model.onnxx")
    ...
    def create_test_model(self) -> ModelProto:
        ...
        reshape = onnxx.helper.make_node(
            "Reshape",
            inputs=["X", "Shape"],
            outputs=["Y"],
        )
        cast = onnxx.helper.make_node(
        ...
        model = helper.make_model(graph_def, producer_name="onnxx-example")
        return model
    ...
    def test_reshape_inference_with_external_data_fail(self) -> None:
        ...
        model_without_external_data = onnxx.load(
    ...
    def test_save_model_with_external_data_multiple_times(self) -> None:
        # Test onnxx.save should respectively handle typical tensor and external tensor properly
        ...
        model_without_loading_external = onnxx.load(
            self.model_file_path, self.serialization_format, load_external_data=False
        )
...
class TestNotAllowToLoadExternalDataOutsideModelDirectory(TestLoadExternalDataBase):
    """Essential test to check that onnxx (validate) C++ code will not allow to load external_data outside the model
    directory.
    """
...
class TestFunctionsAndSubGraphs(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self._model_file_path: str = os.path.join(temp_dir, "model.onnxx")
