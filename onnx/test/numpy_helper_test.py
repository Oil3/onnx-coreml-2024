...
from onnxx import helper, numpy_helper
...
class TestNumpyHelper(unittest.TestCase):
    ...
    def test_from_dict_values_are_np_arrays_of_float(self):
        ...
        self.assertIsInstance(map_proto, onnxx.MapProto)
        self.assertEqual(
    ...
    def _to_array_from_array(self, value: int, check_dtype: bool = True):
        onnx_model = helper.make_model(
            helper.make_graph(
                ...
                "test",
                [helper.make_tensor_value_info("X", onnxx.TensorProto.FLOAT, [4])],
                [helper.make_tensor_value_info("Y", value, [4])],
            )
        )
        ref = onnxx.reference.ReferenceEvaluator(onnx_model)
    ...
    @parameterized.parameterized.expand([(att,) for att in dir(onnxx.TensorProto)])
    def test_to_array_from_array(self, att):
        ...
        if att[0] < "A" or att[0] > "Z":
            return
        value = getattr(onnxx.TensorProto, att)
    ...
    def test_to_array_from_array_subtype(self):
        self._to_array_from_array(onnxx.TensorProto.INT4)
        self._to_array_from_array(onnxx.TensorProto.UINT4)

    def test_to_array_from_array_string(self):
        self._to_array_from_array(onnxx.TensorProto.STRING, False)
