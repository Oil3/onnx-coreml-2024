...
import onnxx
from onnxx import TensorProto, helper, numpy_helper
from onnxx.defs import onnx_opset_version
from onnxx.reference import ReferenceEvaluator
from onnxx.tools import update_model_dims
from onnxx.tools.replace_constants import replace_initializer_by_constant_of_shape
...
class TestToolsFunctions(unittest.TestCase):
    def test_update_inputs_outputs_dim(self) -> None:
        ...
        onnxx.checker.check_model(updated_def)
        self.assertEqual(
    ...
    def test_replace_constant_graph(self):
        ...
        X = helper.make_tensor_value_info("X", onnxx.TensorProto.FLOAT, [None, None])
        Y = helper.make_tensor_value_info("Y", onnxx.TensorProto.FLOAT, [None])

        rsum = helper.make_node("ReduceSum", ["X"], ["rsum"])
        cond = helper.make_node("Greater", ["rsum", "zero"], ["cond"])

        then_out = helper.make_tensor_value_info(
            "then_out", onnxx.TensorProto.FLOAT, None
        )
        ...
        else_out = helper.make_tensor_value_info(
            "else_out", onnxx.TensorProto.FLOAT, None
        )
