...
class TestChecker(unittest.TestCase):
    ...
    def test_check_graph_ir_version_3(self) -> None:
        ...
        ctx.ir_version = 3
        ctx.opset_imports = {"": onnxx.defs.onnx_opset_version()}
    ...
    def test_empty_list_attribute(self):
        model = onnxx.parser.parse_model(
            """
            <
                ir_version: 7,
                opset_import: [ "" : 17]
    ...
    def test_check_model_supports_unicode_path(self):
        input_tensor = helper.make_tensor_value_info(
            "input", onnxx.TensorProto.FLOAT, [1]
        )
        output_tensor = helper.make_tensor_value_info(
            "output", onnxx.TensorProto.FLOAT, [1]
        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            unicode_model_path = os.path.join(temp_dir, "模型モデル모델✨.onnxx")
            onnxx.save(model, unicode_model_path)
