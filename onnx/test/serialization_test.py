...
class _OnnxTestTextualSerializer(onnxx.serialization.ProtoSerializer):
    ...
    def serialize_proto(self, proto) -> bytes:
        text = onnxx.printer.to_text(proto)
        return text.encode("utf-8")

    def deserialize_proto(self, serialized: bytes, proto):
        text = serialized.decode("utf-8")
        if isinstance(proto, onnxx.ModelProto):
            return onnxx.parser.parse_model(text)
        if isinstance(proto, onnxx.GraphProto):
            return onnxx.parser.parse_graph(text)
        if isinstance(proto, onnxx.FunctionProto):
            return onnxx.parser.parse_function(text)
        if isinstance(proto, onnxx.NodeProto):
            return onnxx.parser.parse_node(text)
...
class TestRegistry(unittest.TestCase):
    def setUp(self) -> None:
        ...
        onnxx.serialization.registry.register(self.serializer)

    def test_get_returns_the_registered_instance(self) -> None:
        serializer = onnxx.serialization.registry.get("onnxtext")
        self.assertIs(serializer, self.serializer)

    def test_get_raises_for_unsupported_format(self) -> None:
        with self.assertRaises(ValueError):
            onnxx.serialization.registry.get("unsupported")

    def test_onnx_save_load_model_uses_the_custom_serializer(self) -> None:
        model = onnxx.parser.parse_model(_TEST_MODEL)
        with tempfile.TemporaryDirectory() as tmpdir:
            model_path = os.path.join(tmpdir, "model.onnxx")
            onnxx.save_model(model, model_path, format="onnxtext")

            # Check the file content
            with open(model_path, encoding="utf-8") as f:
                content = f.read()
                self.assertEqual(content, onnxx.printer.to_text(model))

            loaded_model = onnxx.load_model(model_path, format="onnxtext")
...
class TestCustomSerializer(unittest.TestCase):
    def test_serialize_deserialize_model(self) -> None:
        ...
        deserialized = serializer.deserialize_proto(serialized, onnxx.ModelProto())
        self.assertEqual(
            model.SerializeToString(deterministic=True),
