...
import onnxx.backend.base
import onnxx.backend.test
import onnxx.shape_inference
import onnxx.version_converter
from onnxx.backend.base import Device, DeviceType
...
class InferenceSessionBackendRep(onnxx.backend.base.BackendRep):
...
def _create_inference_session(model: onnxx.ModelProto, device: str):
    if device == "CPU":
    ...
    try:
        ...
        raise RuntimeError(
            f"Unable to create inference session. Model is:\n\n{onnxx.printer.to_text(model)}"
        ) from e
    return session


class InferenceSessionBackend(onnxx.backend.base.Backend):
    @classmethod
    ...
    @classmethod
    def prepare(
        cls, model: onnxx.ModelProto, device: str = "CPU", **kwargs: Any
    ) -> InferenceSessionBackendRep:
        del kwargs  # Unused
        if not isinstance(model, (str, bytes, onnxx.ModelProto)):
    ...
    @classmethod
    def run_model(cls, model: onnxx.ModelProto, inputs, device=None, **kwargs):
...
if ort is not None:
    backend_test = onnxx.backend.test.BackendTest(InferenceSessionBackend, __name__)
    ...
    # The following tests are new with opset 19 and 20, or ai.onnxx.ml 4
