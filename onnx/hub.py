...
if "ONNX_HOME" in os.environ:
    ...
    _ONNX_HUB_DIR = join(os.environ["XDG_CACHE_HOME"], "onnxx", "hub")
else:
    _ONNX_HUB_DIR = join(os.path.expanduser("~"), ".cache", "onnxx", "hub")
...
def _verify_repo_ref(repo: str) -> bool:
    ...
    A model repo can be trusted if it matches onnxx/models:main.
    """
    repo_owner, repo_name, repo_ref = _parse_repo_info(repo)
    return (repo_owner == "onnxx") and (repo_name == "models") and (repo_ref == "main")
...
def get_model_info(
    model: str, repo: str = "onnxx/models:main", opset: int | None = None
) -> ModelInfo:
    """Gets the model info matching the given name and opset.

    Args:
        model: The name of the onnxx model in the manifest. This field is
            case-sensitive
        repo: The location of the model repo in format
            "user/repo[:branch]". If no branch is found will default to
            "main"
...
def load(
    ...
    silent: bool = False,
) -> onnxx.ModelProto | None:
    """Downloads a model by name from the onnxx model hub.
    ...
    return onnxx.load(cast(IO[bytes], BytesIO(model_bytes)))
...
def download_model_with_test_data(
    ...
    """Downloads a model along with test data by name from the onnxx model hub and returns the directory to which the files have been extracted.
    ...
    onnxx.utils._extract_model_safe(
...
def load_composite_model(
    ...
    network_repo: str = "onnxx/models:main",
    preprocessing_repo: str = "onnxx/models:main",
    opset: int | None = None,
    force_reload: bool = False,
    silent: bool = False,
) -> onnxx.ModelProto | None:
    ...
    Args:
        network_model: The name of the onnxx model in the manifest.
    ...
    for opset_import_entry in network.opset_import:
        domain = (
            "ai.onnxx" if opset_import_entry.domain == "" else opset_import_entry.domain
        )
    ...
    model_with_preprocessing = onnxx.compose.merge_models(
