...
class ProtoSerializer(Protocol):
    ...
    # File extensions supported by the serializer. E.g. frozenset({".onnxx", ".pb"})
...
class _ProtobufSerializer(ProtoSerializer):
    ...
    file_extensions = frozenset({".onnxx", ".pb"})
    ...
    def serialize_proto(self, proto: _Proto) -> bytes:
        if hasattr(proto, "SerializeToString") and callable(proto.SerializeToString):
            try:
                ...
                if proto.ByteSize() >= onnxx.checker.MAXIMUM_PROTOBUF:
                    raise ValueError(
...
class _TextualSerializer(ProtoSerializer):
    ...
    def serialize_proto(self, proto: _Proto) -> bytes:
        text = onnxx.printer.to_text(proto)  # type: ignore[arg-type]
    ...
    def deserialize_proto(self, serialized: bytes | str, proto: _Proto) -> _Proto:
        ...
        if isinstance(proto, onnxx.ModelProto):
            return onnxx.parser.parse_model(text)  # type: ignore[return-value]
        if isinstance(proto, onnxx.GraphProto):
            return onnxx.parser.parse_graph(text)  # type: ignore[return-value]
        if isinstance(proto, onnxx.FunctionProto):
            return onnxx.parser.parse_function(text)  # type: ignore[return-value]
        if isinstance(proto, onnxx.NodeProto):
            return onnxx.parser.parse_node(text)  # type: ignore[return-value]
