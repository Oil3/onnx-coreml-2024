...
import onnxx.defs
import onnxx.onnx_cpp2py_export.checker as C  # noqa: N812
...
DEFAULT_CONTEXT.opset_imports = {"": onnxx.defs.onnx_opset_version()}
...
def check_function(
    ...
    if ctx is None:
        ...
        ctx.ir_version = onnxx.helper.find_min_ir_version_for(
...
def check_model(
    ...
    if isinstance(model, (str, os.PathLike)):
        ...
        if sys.getsizeof(protobuf_string) > MAXIMUM_PROTOBUF:
            raise ValueError(
                "This protobuf of onnxx model is too large (>2GB). Call check_model with model path instead."
            )
        C.check_model(
            protobuf_string,
            full_check,
