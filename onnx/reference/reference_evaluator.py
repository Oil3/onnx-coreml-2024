...
from onnxx import load
from onnxx.defs import onnx_opset_version
from onnxx.external_data_helper import ExternalDataInfo, uses_external_data
from onnxx.model_container import ModelContainer
from onnxx.onnx_pb import (
    FunctionProto,
    GraphProto,
    ModelProto,
    NodeProto,
...
from onnxx.reference.ops_optimized import optimized_operators
...
class ReferenceEvaluator:
    ...
    Args:
        proto: :class:`onnxx.ModelProto`, :class:`onnxx.GraphProto`,
            :class:`onnxx.FunctionProto`, :class:`onnxx.NodeProto`,
            filename or bytes
    ...
    Next example shows how to run `ReferenceEvaluator` with an onnxx model
    stored in file `model.onnxx`.

    ::

        import numpy as np
        from onnxx.reference import ReferenceEvaluator

        X = np.array(...)
        sess = ReferenceEvaluator("model.onnxx")
    ...
    ::
        ...
        X = np.array(...)
        sess = ReferenceEvaluator("model.onnxx", verbose=1)
        results = sess.run(None, {"X": X})
        print(results[0])  # display the first result
    ...
    `alpha` is an attribute. It can be defined by the onnxx node or
    ...
    .. exec_code::
        ...
        import numpy as np
        from onnxx.reference.ops._op_list import Celu
    ...
    .. exec_code::
        ...
        import numpy as np
        from onnxx.reference.ops import load_op
    ...
    <onnxx.onnx_cpp2py_export.defs.OpSchema>`.

    ::

        from onnxx.reference.op_run.op_conv import Conv as _Conv
        ...
        ::
            ...
            from onnxx.reference.op_run import OpRunExpand
    ...
    def _load_impl(  # noqa: PLR0911
        ...
        if key in self.new_ops_:
            ...
            # This mechanism can be used to implement a custom onnxx node
        ...
        if node.domain == "ai.onnxx.preview.training":
            from onnxx.reference.ops.aionnx_preview_training import load_op as load_op_pt
        ...
        if node.domain == "experimental":
            from onnxx.reference.ops.experimental import load_op as load_op_exp

            return load_op_exp(
                node.domain, node.op_type, version, evaluator_cls=self.__class__
            )

        if node.domain == "ai.onnxx.ml":
            from onnxx.reference.ops.aionnxml import load_op as load_op_ml
