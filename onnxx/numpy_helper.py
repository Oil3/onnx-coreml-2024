...
from onnxx import MapProto, OptionalProto, SequenceProto, TensorProto, helper, subbyte
from onnxx.external_data_helper import load_external_data_for_tensor, uses_external_data
...
def to_array(tensor: TensorProto, base_dir: str = "") -> np.ndarray:
    """Converts a tensor def object to a numpy array.
    Supports types defined in :mod:`onnxx._custom_element_types`.
...
def from_array(tensor: np.ndarray, name: str | None = None) -> TensorProto:
    ...
    supported type defined in :mod:`onnxx._custom_element_types`.
