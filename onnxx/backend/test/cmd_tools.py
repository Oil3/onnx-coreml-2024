...
import onnxx.backend.test.case.model as model_test
import onnxx.backend.test.case.node as node_test
from onnxx import ONNX_ML, TensorProto, numpy_helper
...
def generate_data(args: argparse.Namespace) -> None:
    ...
    for case in cases:
        ...
        if case.kind == "real":
            ...
            assert case.model
            with open(os.path.join(output_dir, "model.onnxx"), "wb") as f:
    ...
    if not args.clean and node_number != original_dir_number:
        warnings.warn(
            "There are some models under 'onnxx/backend/test/data/node' which cannot not"
            " be generated by the script from 'onnxx/backend/test/case/node'. Please add"
            " '--clean' option for 'python onnxx/backend/test/cmd_tools.py generate-data'"
