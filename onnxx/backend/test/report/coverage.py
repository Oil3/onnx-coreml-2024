...
import onnxx
from onnxx import GraphProto, defs, helper
...
class AttrCoverage:
    ...
    def add(self, attr: onnxx.AttributeProto) -> None:
...
class NodeCoverage:
    ...
    def add(self, node: onnxx.NodeProto) -> None:
...
class ModelCoverage:
    ...
    def add(self, model: onnxx.ModelProto) -> None:
...
class Coverage:
    ...
    def add_node(self, node: onnxx.NodeProto, bucket: str) -> None:
        self.buckets[bucket][node.op_type].add(node)

    def add_graph(self, graph: onnxx.GraphProto, bucket: str) -> None:
        for node in graph.node:
            self.add_node(node, bucket)

    def add_model(self, model: onnxx.ModelProto, bucket: str, is_model: bool) -> None:
    ...
    def add_proto(self, proto: onnxx.ModelProto, bucket: str, is_model: bool) -> None:
        assert isinstance(proto, onnxx.ModelProto)
        self.add_model(proto, bucket, is_model)

    def report_text(self, writer: IO[str]) -> None:
        writer.write("---------- onnxx coverage: ----------\n")
        writer.write(
    ...
    # the Backend Scoreboard (onnxx.ai/backend-scoreboard). To enable this
