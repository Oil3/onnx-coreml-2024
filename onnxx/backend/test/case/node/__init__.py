...
import onnxx
from onnxx.backend.test.case.test_case import TestCase
from onnxx.backend.test.case.utils import import_recursive
...
_TargetOpType = None
_DiffOpTypes = None
_existing_names: dict[str, onnxx.NodeProto] = {}
...
def function_testcase_helper(
    ...
    schema = onnxx.defs.get_schema(test_op, domain=node.domain)
...
def _extract_value_info(
    ...
    if type_proto is None:
        if input is None:
            ...
            elem_type = onnxx.helper.np_dtype_to_tensor_dtype(input[0].dtype)
            shape = None
            tensor_type_proto = onnxx.helper.make_tensor_type_proto(elem_type, shape)
            type_proto = onnxx.helper.make_sequence_type_proto(tensor_type_proto)
        elif isinstance(input, TensorProto):
            elem_type = input.data_type
            shape = tuple(input.dims)
            type_proto = onnxx.helper.make_tensor_type_proto(elem_type, shape)
        else:
            elem_type = onnxx.helper.np_dtype_to_tensor_dtype(input.dtype)
            shape = input.shape
            type_proto = onnxx.helper.make_tensor_type_proto(elem_type, shape)

    return onnxx.helper.make_value_info(name, type_proto)
...
def expect(
    ...
    **kwargs: Any,
    ...
    if "opset_imports" not in kwargs:
        ...
        produce_opset_version = onnxx.defs.get_schema(
            node.op_type, domain=node.domain
        ).since_version
        kwargs["opset_imports"] = [
            onnxx.helper.make_operatorsetid(node.domain, produce_opset_version)
        ]
    ...
    def merge(
        node_inputs: list[str], present_value_info: list[onnxx.ValueInfoProto]
    ) -> list[TypeProto]:
        if node_inputs:
            if node_inputs[0] != "":
                return [
    ...
    for expanded_function_nodes, func_opset_import in expanded_tests:
        ...
        if "opset_imports" in kwargs:
            onnx_ai_opset_imports = [
                oi for oi in kwargs["opset_imports"] if oi.domain in ("", "ai.onnxx")
            ]
...
def get_diff_op_types():
    ...
    for file in diff_list:
        ...
        if file_name.startswith("onnxx/backend/test/case/node/") and file_name.endswith(
            ".py"
        ):
