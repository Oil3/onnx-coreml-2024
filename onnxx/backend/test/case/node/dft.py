...
class DFT(Base):
    ...
    def export_opset19() -> None:
        node = onnxx.helper.make_node("DFT", inputs=["x"], outputs=["y"], axis=1)
        ...
        expect(
            ...
            opset_imports=[onnxx.helper.make_opsetid("", 19)],
        )

        node = onnxx.helper.make_node("DFT", inputs=["x"], outputs=["y"], axis=2)
        x = np.arange(0, 100).reshape(10, 10).astype(np.float32)
    ...
    @staticmethod
    def export() -> None:
        node = onnxx.helper.make_node("DFT", inputs=["x", "", "axis"], outputs=["y"])
        x = np.arange(0, 100).reshape(10, 10).astype(np.float32)
