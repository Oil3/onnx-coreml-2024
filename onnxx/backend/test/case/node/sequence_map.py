...
class SequenceMap(Base):
    ...
    def export_sequence_map_identity_1_sequence():  # type: () -> None
        body = onnxx.helper.make_graph(
            [onnxx.helper.make_node("Identity", ["in0"], ["out0"])],
            "seq_map_body",
            [onnxx.helper.make_tensor_value_info("in0", onnxx.TensorProto.FLOAT, ["N"])],
            [onnxx.helper.make_tensor_value_info("out0", onnxx.TensorProto.FLOAT, ["M"])],
        )
        ...
        input_type_protos = [
            onnxx.helper.make_sequence_type_proto(
                onnxx.helper.make_tensor_type_proto(onnxx.TensorProto.FLOAT, ["N"])
            ),
        ]
        output_type_protos = [
            onnxx.helper.make_sequence_type_proto(
    ...
    def export_sequence_map_identity_2_sequences():  # type: () -> None
        body = onnxx.helper.make_graph(
            [
                onnxx.helper.make_node("Identity", ["in0"], ["out0"]),
                onnxx.helper.make_node("Identity", ["in1"], ["out1"]),
            ],
            "seq_map_body",
            [
                onnxx.helper.make_tensor_value_info(
                    "in0", onnxx.TensorProto.FLOAT, ["N"]
                ),
                onnxx.helper.make_tensor_value_info(
                    "in1", onnxx.TensorProto.FLOAT, ["M"]
                ),
            ],
            [
                onnxx.helper.make_tensor_value_info(
                    "out0", onnxx.TensorProto.FLOAT, ["N"]
                ),
                onnxx.helper.make_tensor_value_info(
                    "out1", onnxx.TensorProto.FLOAT, ["M"]
                ),
            ],
        )
        ...
        input_type_protos = [
            ...
            onnxx.helper.make_sequence_type_proto(
                onnxx.helper.make_tensor_type_proto(onnxx.TensorProto.FLOAT, ["M"])
            ),
        ]
        output_type_protos = [
            onnxx.helper.make_sequence_type_proto(
    ...
    def export_sequence_map_add_2_sequences():  # type: () -> None
        body = onnxx.helper.make_graph(
            [onnxx.helper.make_node("Add", ["in0", "in1"], ["out0"])],
            "seq_map_body",
            [
                onnxx.helper.make_tensor_value_info(
                    "in0", onnxx.TensorProto.FLOAT, ["N"]
                ...
                onnxx.helper.make_tensor_value_info(
                    "in1", onnxx.TensorProto.FLOAT, ["N"]
                ),
            ],
            [onnxx.helper.make_tensor_value_info("out0", onnxx.TensorProto.FLOAT, ["N"])],
        )
    ...
    def export_sequence_map_extract_shapes():  # type: () -> None
        body = onnxx.helper.make_graph(
            [onnxx.helper.make_node("Shape", ["x"], ["shape"])],
            "seq_map_body",
            [
                onnxx.helper.make_tensor_value_info(
            ...
            [onnxx.helper.make_tensor_value_info("shape", onnxx.TensorProto.INT64, [3])],
        )
        ...
        input_type_protos = [
            onnxx.helper.make_sequence_type_proto(
                onnxx.helper.make_tensor_type_proto(
                    onnxx.TensorProto.FLOAT, ["H", "W", "C"]
                )
            ),
        ]
        output_type_protos = [
            onnxx.helper.make_sequence_type_proto(
                onnxx.helper.make_tensor_type_proto(onnxx.TensorProto.INT64, [3])
            ),
        ]
        expect(
            node,
