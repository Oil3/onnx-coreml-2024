...
class Sequence(Base):
    ...
    def export() -> None:
        def make_graph(
            nodes: list[onnxx.helper.NodeProto],
            ...
            graph = onnxx.helper.make_graph(
                ...
                name="Sequence",
                inputs=[
                    onnxx.helper.make_tensor_value_info(name, input_type, input_shape)
                    for name, input_type, input_shape in zip(
                        input_names, input_types, input_shapes
                    )
                ],
                outputs=[
                    onnxx.helper.make_tensor_value_info(name, output_type, output_shape)
        ...
        # 5. SequenceAt(2):         -> y
        seq_empty_node = onnxx.helper.make_node("SequenceEmpty", [], ["Seq_empty"])
        seq_insert_node = onnxx.helper.make_node(
            "SequenceInsert", ["Seq_empty", "X"], ["Seq_1"]
        )
        seq_insert_node2 = onnxx.helper.make_node(
            "SequenceInsert", ["Seq_1", "Y"], ["Seq_2"]
        )
        seq_insert_node3 = onnxx.helper.make_node(
            "SequenceInsert", ["Seq_2", "Z", "pos"], ["Seq_3"]
        )
        seq_at_node = onnxx.helper.make_node("SequenceAt", ["Seq_3", "pos_at"], ["out"])
        ...
        pos = onnxx.helper.make_tensor("pos", TensorProto.INT64, (), (pos_val,))
        pos_at = onnxx.helper.make_tensor("pos_at", TensorProto.INT64, (), (pos_at_val,))
        ...
        graph = make_graph(
            ...
            ["X", "Y", "Z", "pos", "pos_at"],
            ["out"],
            [onnxx.TensorProto.FLOAT] * 3 + [onnxx.TensorProto.INT64] * 2,  # type: ignore
        ...
        model = onnxx.helper.make_model_gen_version(
            ...
            opset_imports=[onnxx.helper.make_opsetid("", 12)],
        )
        ...
        # 3. SequenceAt(1):                 -> z
        seq_construct_node = onnxx.helper.make_node(
            "SequenceConstruct", ["X", "Y", "Z"], ["seq_1"]
        )
        seq_erase_node = onnxx.helper.make_node(
            "SequenceErase", ["seq_1", "pos_erase"], ["seq_2"]
        )
        seq_at_node = onnxx.helper.make_node("SequenceAt", ["seq_2", "pos_at"], ["out"])
        ...
        pos_erase = onnxx.helper.make_tensor(
        ...
        pos_insert = onnxx.helper.make_tensor(
        ...
        graph = make_graph(
            ...
            ["out"],
            [onnxx.TensorProto.FLOAT] * 3 + [onnxx.TensorProto.INT64] * 3,  # type: ignore
            [onnxx.TensorProto.FLOAT],
        ...
        seq_concat_node = onnxx.helper.make_node(
        ...
        graph = make_graph(
            ...
            ["X", "Y", "Z"],
            ["out"],
            [onnxx.TensorProto.FLOAT] * 3,  # type: ignore
            [onnxx.TensorProto.FLOAT],
        )
        model = onnxx.helper.make_model_gen_version(
            graph,
        ...
        seq_split_node = onnxx.helper.make_node(
            "SplitToSequence", ["X"], ["seq_1"], axis=-1
        )
        seq_len_node = onnxx.helper.make_node("SequenceLength", ["seq_1"], ["len"])
        ...
        graph = onnxx.helper.make_graph(
            ...
            inputs=[
                onnxx.helper.make_tensor_value_info(
                    "X", onnxx.TensorProto.FLOAT, tensor_shape
                )
            ],
            outputs=[
                onnxx.helper.make_tensor_value_info(
                    "len", onnxx.TensorProto.INT64, len_shape
                )
            ],
        )  # type: ignore
        ...
        seq_at_node = onnxx.helper.make_node("SequenceAt", ["seq_1", "pos_at"], ["out"])
        ...
        graph = make_graph(
            ...
            ["X", "pos_at"],
            ["out"],
            [onnxx.TensorProto.DOUBLE, onnxx.TensorProto.INT64],
            [onnxx.TensorProto.DOUBLE],
            [pos_at],
        )
        model = onnxx.helper.make_model_gen_version(
            graph,
        ...
        graph = onnxx.helper.make_graph(
            ...
            inputs=[
                ...
                onnxx.helper.make_tensor_value_info(
                    "Splits", onnxx.TensorProto.INT64, splits_shape
                ),
            ],  # type: ignore
            outputs=[
                onnxx.helper.make_tensor_value_info(
