...
def merge_graphs(
    ...
    if len(overlapping_names) > 0:
        ...
        raise ValueError(
            ...
            + ", ".join(names)
            + "\n"
            + "Consider using ``onnxx.compose.add_prefix`` to add a prefix to names in one of the graphs."
        )
...
def merge_models(
    ...
    name: str | None = None,
    doc_string: str | None = None,
    producer_name: str | None = "onnxx.compose.merge_models",
    ...
    Arguments:
        ...
        doc_string (string): Optional docstring for the combined graph
                             If not provided, a default docstring with the concatenation of g1 and g2 docstrings is used
        producer_name (string): Optional producer name for the combined model. Default: 'onnxx.compose'
