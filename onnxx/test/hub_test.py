...
from onnxx import ModelProto, hub
...
class TestModelHub(unittest.TestCase):
    ...
    def test_force_reload(self) -> None:
        ...
        cached_files = list(
            glob.glob(join(hub.get_dir(), "**", "*.onnxx"), recursive=True)
        )
    ...
    def test_custom_cache(self) -> None:
        ...
        cached_files = list(glob.glob(join(new_cache, "**", "*.onnxx"), recursive=True))
        self.assertGreaterEqual(len(cached_files), 1)
    ...
    def test_manifest_not_found(self) -> None:
        self.assertRaises(
            AssertionError,
            lambda: hub.load(self.name, "onnxx/models:unknown", silent=True),
        )
    ...
    def test_verify_repo_ref(self) -> None:
        ...
        # Not trusted repo:
        verified = hub._verify_repo_ref("onnxx/models:unknown")
        self.assertFalse(verified)
    ...
    def test_download_model_with_test_data(self) -> None:
        ...
        self.assertIn(member="model.onnxx", container=files, msg="Onnx model not found")
        self.assertIn(
