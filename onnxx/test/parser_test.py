...
import onnxx
from onnxx import GraphProto, OperatorSetIdProto, TensorProto, checker
...
class TestBasicFunctions(unittest.TestCase):
    ...
    def test_parse_graph(self) -> None:
        input = """
           ...
           """
        graph = onnxx.parser.parse_graph(input)
    ...
    def test_parse_graph_error(self) -> None:
        input = """
           ...
           """
        self.assertRaises(
            onnxx.parser.ParseError, lambda: onnxx.parser.parse_graph(input)
        )
    ...
    def test_parse_model_error(self) -> None:
        input = """
           ...
           """
        self.assertRaises(
            onnxx.parser.ParseError, lambda: onnxx.parser.parse_model(input)
        )
    ...
    def test_composite_parse_function_with_attributes(
        ...
        functions = [onnxx.parser.parse_function(function_text)]
        graph = onnxx.parser.parse_graph(graph_text)
        opset_imports = [
            OperatorSetIdProto(domain="", version=15),
            OperatorSetIdProto(domain="custom_domain", version=1),
        ]

        model = onnxx.helper.make_model(
    ...
    def test_parse_node(self):
        ...
        attr_val = onnxx.helper.get_node_attr_value(node, "attr1")
    ...
    def test_parse_various_float_values(self, test_literal, expect_exception):
        ...
        if expect_exception:
            self.assertRaises(
                onnxx.parser.ParseError, lambda: onnxx.parser.parse_model(model_text)
            )
        else:
            model = onnxx.parser.parse_model(model_text)
            ...
            self.assertEqual(
                model.graph.node[0].attribute[0].type, onnxx.AttributeProto.FLOAT
            )
            self.assertEqual(
    ...
    def test_parse_graph_types(self, name, itype) -> None:
        ...
        text_graph = f"""
           ...
           """
        graph = onnxx.parser.parse_model(text_graph)
