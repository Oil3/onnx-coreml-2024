...
class TestUtilityFunctions(unittest.TestCase):
    def test_extract_model(self) -> None:
        ...
        p0 = os.path.join(tdir, "original.onnxx")
        onnxx.save(m0, p0)

        p1 = os.path.join(tdir, "extracted.onnxx")
        input_names = ["B0", "B1", "B2"]
        output_names = ["C0", "C1"]
        onnxx.utils.extract_model(p0, p1, input_names, output_names)

        m1 = onnxx.load(p1)
        self.assertEqual(m1.producer_name, "onnxx.utils.extract_model")
