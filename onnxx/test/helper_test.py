...
from onnxx.reference.op_run import to_array_extended
...
class TestHelperNodeFunctions(unittest.TestCase):
    ...
    def test_model_irversion(self) -> None:
        ...
        test([("", 21)], 10)
        test([("", 22)], 10)
        # standard opset can be referred to using empty-string or "ai.onnxx"
        test([("ai.onnxx", 9)], 4)
        test([("ai.onnxx.ml", 2)], 6)
        test([("ai.onnxx.ml", 3)], 8)
        test([("ai.onnxx.ml", 4)], 9)
        test([("ai.onnxx.ml", 5)], 10)
        test([("ai.onnxx.training", 1)], 7)
        # helper should pick *max* IR version required from all opsets specified.
        test([("", 10), ("ai.onnxx.ml", 2)], 6)
