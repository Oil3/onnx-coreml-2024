...
from onnxx import AttributeProto, TypeProto

float_type_ = onnxx.helper.make_tensor_type_proto(1, None)
uint8_type_ = onnxx.helper.make_tensor_type_proto(2, None)
int8_type_ = onnxx.helper.make_tensor_type_proto(3, None)
int32_type_ = onnxx.helper.make_tensor_type_proto(6, None)
float16_type_ = onnxx.helper.make_tensor_type_proto(10, None)
no_type_ = TypeProto()
...
class TestFunctionInference(TestShapeInferenceHelper):
    def _check(
        ...
        function = onnxx.parser.parse_function(function_text)
        result = onnxx.shape_inference.infer_function_output_types(
    ...
    def _check_fails(
        ...
        def invoke_inference():
            onnxx.shape_inference.infer_function_output_types(
                function, input_types, attributes
            )

        self.assertRaises(onnxx.shape_inference.InferenceError, invoke_inference)
    ...
    def test_fi_attribute(self):
        ...
        """
        dtype_6 = onnxx.helper.make_attribute("dtype", 6)
        self._check(code, [float_type_], [dtype_6], [int32_type_])

        dtype_10 = onnxx.helper.make_attribute("dtype", 10)
