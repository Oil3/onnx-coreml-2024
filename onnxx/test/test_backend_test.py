...
from onnxx import ModelProto, NodeProto, TensorProto
from onnxx.backend.base import Device, DeviceType
from onnxx.backend.test.runner import BackendIsNotSupposedToImplementIt
...
class DummyBackend(onnxx.backend.base.Backend):
    @classmethod
    def prepare(
        cls, model: ModelProto, device: str = "CPU", **kwargs: Any
    ) -> onnxx.backend.base.BackendRep | None:
        super().prepare(model, device, **kwargs)

        onnxx.checker.check_model(model)

        # by default test strict shape inference
        kwargs = {"check_type": True, "strict_mode": True, **kwargs}
        model = onnxx.shape_inference.infer_shapes(model, **kwargs)
...
backend_test = onnxx.backend.test.BackendTest(
