...
import onnxx.version_converter
from onnxx import (
    GraphProto,
    ModelProto,
    OperatorSetIdProto,
...
class TestVersionConverter(unittest.TestCase):
    def _converted(
        ...
        orig_model = helper.make_model(
            graph, producer_name="onnxx-test", opset_imports=[initial_version]
        )
        # print(type(orig_model))
        converted_model = onnxx.version_converter.convert_version(
    ...
    def test_max_7_8(self) -> None:
        ...
        nodes = [onnxx.helper.make_node("Max", inputs=["X"], outputs=["Y"])]

        graph = helper.make_graph(
            nodes,
            "test_max",
            [onnxx.helper.make_tensor_value_info("X", data_type, data_shape)],
            [onnxx.helper.make_tensor_value_info("Y", data_type, data_shape)],
        )
    ...
    def test_min_7_8(self) -> None:
        ...
        nodes = [onnxx.helper.make_node("Min", inputs=["X"], outputs=["Y"])]
    ...
    def test_mean_7_8(self) -> None:
        ...
        nodes = [onnxx.helper.make_node("Mean", inputs=["X"], outputs=["Y"])]
    ...
    def test_upsample_6_7(self) -> None:
        ...
        graph = helper.make_graph(
            nodes,
            "test_upsample_6_7",
            [onnxx.helper.make_tensor_value_info("X", data_type, [1, 1, 2, 2])],
            [onnxx.helper.make_tensor_value_info("Y", data_type, [1, 1, 4, 6])],
        )
    ...
    def test_batchnormalization_9_8(self) -> None:
        ...
        input_shape = (2, 3, 4, 5)
        x = onnxx.helper.make_tensor_value_info("X", data_type, input_shape)
        scale = onnxx.helper.make_tensor_value_info("scale", data_type, [input_shape[1]])
        B = onnxx.helper.make_tensor_value_info("B", data_type, [input_shape[1]])
        mean = onnxx.helper.make_tensor_value_info("mean", data_type, [input_shape[1]])
        var = onnxx.helper.make_tensor_value_info("var", data_type, [input_shape[1]])
        y = onnxx.helper.make_tensor_value_info("Y", data_type, input_shape)
    ...
    def test_constant_8_9(self) -> None:
        ...
        graph = helper.make_graph(
            ...
            "test_constant",
            [],
            [onnxx.helper.make_tensor_value_info("Y", data_type, output_shape)],
        )
    ...
    def test_flatten_8_9(self) -> None:
        ...
        nodes = [onnxx.helper.make_node("Flatten", inputs=["X"], outputs=["Y"], axis=1)]

        graph = helper.make_graph(
            nodes,
            "test_flatten",
            [onnxx.helper.make_tensor_value_info("X", data_type, [2, 3, 4])],
            [onnxx.helper.make_tensor_value_info("Y", data_type, [2, 12])],
        )
    ...
    def test_prelu_8_9(self) -> None:
        ...
        nodes = [onnxx.helper.make_node("PRelu", inputs=["X", "Slope"], outputs=["Y"])]
        ...
        graph = helper.make_graph(
            ...
            "test_prelu",
            [
                onnxx.helper.make_tensor_value_info("X", data_type, input_shape),
                onnxx.helper.make_tensor_value_info("Slope", data_type, input_shape),
            ],
            [onnxx.helper.make_tensor_value_info("Y", data_type, input_shape)],
        )
    ...
    def test_greater_8_9(self) -> None:
        ...
        nodes = [onnxx.helper.make_node("Greater", inputs=["X1", "X2"], outputs=["Y"])]
        ...
        graph = helper.make_graph(
            ...
            "test_greater",
            [
                onnxx.helper.make_tensor_value_info("X1", data_type, input_shape),
                onnxx.helper.make_tensor_value_info("X2", data_type, input_shape),
            ],
            [onnxx.helper.make_tensor_value_info("Y", TensorProto.BOOL, input_shape)],
        )
    ...
    def test_less_8_9(self) -> None:
        ...
        nodes = [onnxx.helper.make_node("Less", inputs=["X1", "X2"], outputs=["Y"])]
    ...
    def test_matmul_8_9(self) -> None:
        ...
        nodes = [onnxx.helper.make_node("MatMul", inputs=["X1", "X2"], outputs=["Y"])]

        graph = helper.make_graph(
            nodes,
            "test_matmul",
            [
                onnxx.helper.make_tensor_value_info("X1", data_type, [3, 4]),
                onnxx.helper.make_tensor_value_info("X2", data_type, [4, 3]),
            ],
            [onnxx.helper.make_tensor_value_info("Y", data_type, [3, 3])],
        )
    ...
    def test_gemm_8_9(self) -> None:
        ...
        nodes = [
            onnxx.helper.make_node("Gemm", inputs=["X1", "X2", "X3"], outputs=["Y"])
        ]
        ...
        graph = helper.make_graph(
            ...
            [
                onnxx.helper.make_tensor_value_info("X1", data_type, [3, 4]),
                onnxx.helper.make_tensor_value_info("X2", data_type, [4, 3]),
                onnxx.helper.make_tensor_value_info("X3", data_type, [3, 3]),
            ],
            [onnxx.helper.make_tensor_value_info("Y", data_type, [3, 3])],
        )
    ...
    def helper_upsample_with_initializer(self, raw_scale: bool = False) -> None:
        ...
        scale_tensor = onnxx.helper.make_tensor(
            "Scales",
            onnxx.TensorProto.FLOAT,
            [4],
        ...
        graph = helper.make_graph(
            ...
            [
                onnxx.helper.make_tensor_value_info("X", data_type, [1, 1, 2, 2]),
                onnxx.helper.make_tensor_value_info("Scales", data_type, [4]),
            ],
            [onnxx.helper.make_tensor_value_info("Y", data_type, [1, 1, 4, 6])],
            [scale_tensor],
        )
    ...
    def helper_upsample_with_constant(self, raw_scale: bool = False) -> None:
        ...
        graph = helper.make_graph(
            ...
            [onnxx.helper.make_tensor_value_info("Y", data_type, [1, 1, 4, 6])],
            value_info=[
                onnxx.helper.make_tensor_value_info("Constant_Output", data_type, [4])
            ],
        )
    ...
    def test_scan_8_9(self) -> None:
        ...
        node1 = onnxx.helper.make_node(
            "Add",
            inputs=["sum_in", "next"],
            outputs=["sum_out"],
        )
        node2 = onnxx.helper.make_node(
            "Identity",
            inputs=["sum_out"],
            outputs=["scan_out"],
        )
        g = onnxx.helper.make_graph(
            [node1, node2],
            "scan_body",
            [
                onnxx.helper.make_tensor_value_info("sum_in", data_type, [2]),
                onnxx.helper.make_tensor_value_info("next", data_type, [2]),
            ],
            [
                onnxx.helper.make_tensor_value_info("sum_out", data_type, [2]),
                onnxx.helper.make_tensor_value_info("scan_out", data_type, [2]),
            ],
        )
        ...
        initial = onnxx.helper.make_tensor_value_info("initial", data_type, [1, 2])
        x = onnxx.helper.make_tensor_value_info("x", data_type, [1, 3, 2])
        y = onnxx.helper.make_tensor_value_info("y", data_type, [1, 2])
        z = onnxx.helper.make_tensor_value_info("z", data_type, [1, 3, 2])

        graph = onnxx.helper.make_graph(nodes, "test_scan_8_9", [initial, x], [y, z])
    ...
    def test_cast_8_9(self) -> None:
        ...
        graph = helper.make_graph(
            nodes,
            "test_cast",
            [onnxx.helper.make_tensor_value_info("X", data_type_from, [2, 3])],
            [onnxx.helper.make_tensor_value_info("Y", data_type_to, [2, 3])],
        )
    ...
    def test_if_subgraph_10_11(self) -> None:
        ...
        subg1_input = [
            onnxx.helper.make_tensor_value_info("sub_in", data_type, data_shape)
        ]
        subg1_output = [
            onnxx.helper.make_tensor_value_info("sub_out", data_type, data_shape)
        ]
        ...
        input = [onnxx.helper.make_tensor_value_info("cond", TensorProto.BOOL, [])]
        output = [onnxx.helper.make_tensor_value_info("out", data_type, data_shape)]
    ...
    def test_initializer_not_in_input_above_ir4(self):  # type: () -> None
        ...
        scale_tensor = onnxx.helper.make_tensor(
            "scale", onnxx.TensorProto.FLOAT, [2], scale_value
        )
        b_value = [0.60, 0.54]
        b_tensor = onnxx.helper.make_tensor("B", onnxx.TensorProto.FLOAT, [2], b_value)
        mean_value = [0.42, 0.65]
        mean_tensor = onnxx.helper.make_tensor(
            "mean", onnxx.TensorProto.FLOAT, [2], mean_value
        )
        var_value = [0.44, 0.89]
        var_tensor = onnxx.helper.make_tensor(
            "var", onnxx.TensorProto.FLOAT, [2], var_value
        )
