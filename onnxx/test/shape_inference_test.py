...
from onnxx.parser import parse_graph
...
class TestShapeInferenceHelper(unittest.TestCase):
    ...
    def _inferred(
        ...
        if isinstance(graph_or_model, GraphProto):
            kwargs["producer_name"] = "onnxx-test"
            orig_model = helper.make_model(graph_or_model, **kwargs)
        else:
            orig_model = graph_or_model
        inferred_model = onnxx.shape_inference.infer_shapes(
    ...
    def _compare_value_infos(
        ...
        elif vi_type == onnxx.TypeProto():
            assert inferred_vi_type == onnxx.TypeProto()
        else:
...
class TestShapeInference(TestShapeInferenceHelper):
    def test_empty_graph(self) -> None:
        ...
        self.assertRaises(onnxx.shape_inference.InferenceError, self._inferred, graph)
    ...
    def test_upsample(self, _, version) -> None:
        if version == 7:
            ...
            if version == 9:
                ...
                with self.assertRaises(onnxx.checker.ValidationError) as cm:
                    call_inference()
    ...
    def test_upsample_raw_data(self, _, version) -> None:
        if version == 7:
            ...
            )  # Feed raw bytes (force little endian ordering like onnxx standard) for test purpose
    ...
    def test_conv_transpose_with_pads_and_auto_pads(self) -> None:
        ...
        self.assertRaises(
            onnxx.shape_inference.InferenceError,
            onnxx.shape_inference.infer_shapes,
    ...
    @parameterized.expand(
        [onnxx.TensorProto.FLOAT, onnxx.TensorProto.FLOAT16, onnxx.TensorProto.BFLOAT16]
    )
    ...
    @unittest.skipUnless(ONNX_ML, "ONNX_ML required to test ai.onnxx.ml operators")
    ...
    def test_label_encoder_string_int64(self, _, version) -> None:
        self.skipIf(
            version < 2, "keys_* attributes were introduced in ai.onnxx.ml opset 2"
        )
        string_list = ["A", "m", "y"]
    ...
    def test_label_encoder_tensor_attributes(self, _, version) -> None:
        self.skipIf(
            version < 4, "tensor attributes were introduced in ai.onnxx.ml opset 4"
        )
    ...
    def test_label_encoder_tensor_attributes_invalid_configurations(
        ...
        self.skipIf(version < 4, "tensor attributes introduced in ai.onnxx.ml opset 4")
        key_tensor = make_tensor(
            "keys_tensor", TensorProto.STRING, [4], ["a", "b", "cc", "ddd"]
        )
        values_tensor = make_tensor(
    ...
    def test_infer_initializer_input_consistency_all_none(self) -> None:
        ...
        onnxx.shape_inference.infer_shapes(original_model, strict_mode=True)
    ...
    def test_parse_data_with_unsupported_tensor_type(self) -> None:
        ...
        inferred_model = onnxx.shape_inference.infer_shapes(model)
        self.assertFalse(
        ...
        body_graph = helper.make_graph(
            ...
            inputs=[
                onnxx.helper.make_tensor_value_info(
                    "x", onnxx.TensorProto.FLOAT, ("H", "W", 3)
                )
            ],
            outputs=[
                onnxx.helper.make_tensor_value_info(
                    "y1", onnxx.TensorProto.FLOAT, (10, 20, 3)
                ),
                onnxx.helper.make_tensor_value_info(
                    "y2", onnxx.TensorProto.FLOAT, (30, 40, 3)
                ),
            ],
            initializer=[
                make_tensor("axes", TensorProto.INT64, (2,), (0, 1)),
    ...
    def test_sequence_map_slice_outs_unknown_dims(self):
        body_graph = helper.make_graph(
            ...
            outputs=[
                onnxx.helper.make_tensor_value_info(
                    "y1", onnxx.TensorProto.FLOAT, ("H1", "W1", 3)
                ),
                onnxx.helper.make_tensor_value_info(
                    "y2", onnxx.TensorProto.FLOAT, ("H2", "W2", 3)
                ),
            ],
            initializer=[
                make_tensor("axes", TensorProto.INT64, (2,), (0, 1)),
    ...
    def test_sequence_map_different_tensor_type(self):
        body_graph = helper.make_graph(
            ...
            inputs=[
                onnxx.helper.make_tensor_value_info(
                    "x", onnxx.TensorProto.FLOAT, ("H", "W", "C")
                )
            ],
            outputs=[
                onnxx.helper.make_tensor_value_info(
                    "shape", onnxx.TensorProto.INT64, (3,)
                )
            ],
        )  # type: ignore
    ...
    def test_dft_invalid_axis_opset17(self, _: str, axis: int) -> None:
        ...
        with self.assertRaises(onnxx.shape_inference.InferenceError):
            self._assert_inferred(
                graph,
                [
    ...
    def test_zip_map(self) -> None:
        params = (
            ({"classlabels_int64s": [1, 2, 3]}, onnxx.TensorProto.INT64),
            ({"classlabels_strings": ["a", "b", "c"]}, onnxx.TensorProto.STRING),
        )
    ...
    def zip_map_test_case(self, attrs, input_type) -> None:
        ...
        typ = onnxx.helper.make_map_type_proto(
            input_type, onnxx.helper.make_tensor_type_proto(TensorProto.FLOAT, ())
        )
        self._assert_inferred(
            graph,
            [
                onnxx.helper.make_value_info(
                    "output", onnxx.helper.make_sequence_type_proto(typ)
                )
            ],
            opset_imports=[
                make_opsetid(ONNX_ML_DOMAIN, 1),
    ...
    def test_check_type_when_schema_has_empty_io(self):
        input = """
           ...
           """
        model = onnxx.parser.parse_model(input)
        ...
        onnxx.defs.register_schema(op_schema)
        with self.assertRaises(onnxx.shape_inference.InferenceError):
            onnxx.shape_inference.infer_shapes(model, True)
        onnxx.defs.deregister_schema(
    ...
    def test_issue_layer_normalization_6187(self):
        ...
        """
        model = onnxx.parser.parse_model(modeltxt)
        with self.assertRaises(onnxx.shape_inference.InferenceError):
            onnxx.checker.check_model(model, full_check=True)
            onnxx.shape_inference.infer_shapes(model)
    ...
    def test_issue_gemm_6185(self):
        ...
        """
        model = onnxx.parser.parse_model(modeltxt)
        with self.assertRaises(onnxx.checker.ValidationError):
            onnxx.checker.check_model(model, full_check=True)
            onnxx.shape_inference.infer_shapes(model)
