...
class TestModelInference(unittest.TestCase):
    def _check(self, model_text: str, *expected: int):
        ...
        only the element type (ints corresponding to onnxx.TensorProto.DataType).
        """
        model = onnxx.parser.parse_model(model_text)
        inferred = onnxx.shape_inference.infer_shapes(model)
    ...
    def _check_inference_error(self, model_text: str):
        ...
        with self.assertRaises(onnxx.shape_inference.InferenceError):
            onnxx.shape_inference.infer_shapes(model, True, True)
    ...
    def test_mi_basic(self):
        ...
        """
        self._check(model, onnxx.TensorProto.INT32)
    ...
    def test_mi_function_multiple_calls(self):
        ...
        """
        self._check(model, onnxx.TensorProto.INT32, onnxx.TensorProto.INT64)
    ...
    def _check_shape(self, model_text: str, *expected: typing.Sequence[int]):
        ...
        """
        model = onnxx.parser.parse_model(model_text)
        inferred = onnxx.shape_inference.infer_shapes(model, True, True, True)
        outputs = inferred.graph.output
    ...
    def test_mi_function_default_attr(self):
        ...
        """
        self._check(model, onnxx.TensorProto.INT32, onnxx.TensorProto.FLOAT)
