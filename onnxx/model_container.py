...
import onnxx.helper
import onnxx.onnx_cpp2py_export.checker as c_checker
...
class ModelContainer:
    ...
    def __init__(self):
        self.model_proto_: onnxx.ModelProto | None = None
        self.large_initializers: dict[str, np.ndarray] = {}

    def check_model(self):
        if self.model_proto is not None:
            onnxx.checker.check_model(self.model_proto)
    ...
    def check_large_initializers(self):
        for tensor in ext_data._get_all_tensors(self.model_proto):
            if not ext_data.uses_external_data(tensor):
                continue
            prop: onnxx.StringStringEntryProto | None = None
    ...
    def _save_external(
        ...
        """Save the large model into a main onnxx file and one file
        per tensor. Follows the same format as :func:`write_external_data_tensors
        <onnxx.external_data_helper.write_external_data_tensors>`.
    ...
    def load(self, file_path: str, load_large_initializers: bool = True):
        ...
        """
        self.model_proto_ = onnxx.load_model(file_path, load_external_data=False)
    ...
    def _load_large_initializers(self, file_path):
        ...
        for i, tensor in enumerate(ext_data._get_all_tensors(self.model_proto_)):
            ...
            with open(external_data_file_path, "rb") as data_file:
                ...
                dtype = onnxx.helper.tensor_dtype_to_np_dtype(tensor.data_type)
...
def make_large_model(
    ...
    **kwargs: Any,
    ...
    Returns:
        ModelContainer
    """
    model = onnxx.helper.make_model(graph, **kwargs)
