...
import onnxx._custom_element_types as custom_np_types
from onnxx import (
    IR_VERSION,
    AttributeProto,
    FunctionProto,
...
VERSION_TABLE: VersionTableType = [
    # Release-version, IR version, ai.onnxx version, ai.onnxx.ml version, (optional) ai.onnxx.training version
    ("1.0", 3, 1, 1),
    ("1.1", 3, 5, 1),
    ("1.1.2", 3, 6, 1),
    ("1.2", 3, 7, 1),
...
def create_op_set_id_version_map(table: VersionTableType) -> VersionMapType:
    ...
    def process(release_version: str, ir_version: int, *args: Any) -> None:
        ...
        for pair in zip(["ai.onnxx", "ai.onnxx.ml", "ai.onnxx.training"], args):
            if pair not in result:
                result[pair] = ir_version
                if pair[0] == "ai.onnxx.training":
                    result["ai.onnxx.preview.training", pair[1]] = ir_version
...
def find_min_ir_version_for(
    ...
    def find_min(domain: str | None, version: int) -> int:
        key = (domain or "ai.onnxx", version)
...
def float32_to_float8e4m3(  # noqa: PLR0911
    ...
    See :ref:`onnxx-detail-float8` for technical details.
...
def pack_float32_to_4bit(array: np.ndarray | Sequence, signed: bool) -> np.ndarray:
    ...
    See :ref:`onnxx-detail-int4` for technical details.
...
def make_tensor(
    ...
    Args:
        ...
        data_type (int): a value such as onnxx.TensorProto.FLOAT
        dims (List[int]): shape
        vals: values
