...
import onnxx.checker
import onnxx.helper
import onnxx.shape_inference
from onnxx import FunctionProto, ModelProto, NodeProto, TensorProto, ValueInfoProto


class Extractor:
    def __init__(self, model: ModelProto) -> None:
        self.model = onnxx.shape_inference.infer_shapes(model)
    ...
    def _make_model(
        ...
        meta = {
            ...
            "producer_name": "onnxx.utils.extract_model",
            "functions": local_functions,
        }
        return onnxx.helper.make_model(graph, **meta)
...
def extract_model(
    ...
    onnxx.checker.check_model(input_path)
    model = onnxx.load(input_path)

    e = Extractor(model)
    extracted = e.extract_model(input_names, output_names)

    onnxx.save(extracted, output_path)
    if check_model:
        onnxx.checker.check_model(output_path)
