...
from coremltoolsx.models._deprecation import deprecated as _deprecated
from coremltoolsx.models.neural_network import NeuralNetworkBuilder  # type: ignore
from coremltoolsx.models import datatypes, MLModel  # type: ignore
...
from coremltoolsx import _MINIMUM_NDARRAY_SPEC_VERSION as IOS_13_SPEC_VERSION  # iOS 13.0
from coremltoolsx import __version__ as ct_version
from coremltoolsx.models import _METADATA_VERSION, _METADATA_SOURCE

from coremltoolsx._deps import _HAS_ONNX

# ML model passes
from coremltoolsx.converters.mil.backend.nn.passes.mlmodel_passes import (
...
if _HAS_ONNX:
    import onnxx
    from onnxx import shape_inference
...
def _make_coreml_input_features(
    graph, onnx_coreml_input_shape_map, disable_coreml_rank5_mapping=True
    ...
    onnxx shapes are mapped "as is" to CoreML.
    """
    inputs = graph.inputs
...
def _make_coreml_output_features(
    graph, forceShape=False, disable_coreml_rank5_mapping=True
):  # type: (...) -> Sequence[Tuple[Text, datatypes.Array]]
    features = []
...
def _check_unsupported_ops(
    nodes, disable_coreml_rank5_mapping=True
):  # type: (...) -> None
    ...
    if not disable_coreml_rank5_mapping:
        coreml_3_rerun_message = (
            ...
            " and making sure you have the latest coremltoolsx package"
        )
...
def convert(
    model,  # type: Union[onnxx.ModelProto, Text]
    ...
    custom_conversion_functions: dict()
        * A dictionary with keys corresponding to the names/types of ONNX ops and values as
          ...
          ``'NeuralNetworkBuilder'``, ``'Graph'`` (see ``onnxx-coreml/_graph.Graph``),
          ``'Node'`` (see ``onnxx-coreml/_graph.Node``), and
          ``'ErrorHandling'`` (see ``onnxx-coreml/_error_utils.ErrorHandling``).
    ...
    if isinstance(model, Text):
        ...
        raise TypeError("Model must be file path to .onnxx file or onnxx loaded model")
    ...
    onnx_model = onnxx.shape_inference.infer_shapes(onnx_model)
    ...
    This information is then used to initialize the neural network builder object of coremltoolsx.
    ...
    """
    Set CoreML input,output types (float, double, int) same as onnxx types, if supported
    """
    ...
    try:
        if DEBUG:
            ...
            coremltoolsx.models.utils.save_spec(
                builder.spec, "/tmp/node_model_raw_spec.mlmodel"
            )
            from coremltoolsx.models.neural_network.printer import print_network_spec
    ...
    if len(err.custom_layer_nodes) > 0:
        ...
        print(
            ...
            "corresponding to the following ops in the onnxx model: "
        )
    ...
    mlmodel.user_defined_metadata[_METADATA_SOURCE] = "onnxx=={0}".format(
        onnxx.__version__
    )
    return mlmodel
