...
from onnxx import numpy_helper, ValueInfoProto, AttributeProto, GraphProto, NodeProto, TensorProto, TensorShapeProto
...
class Graph(object):
    def __init__(self,
                 ...
                 initializers, # type: Dict[Text, np._ArrayLike[Any]]
        ...
        self.onnx_coreml_name_map = {} # map from original name to effective name after Identity removal
        self.coreml_onnx_name_map = {} # map from effective name back to original name
        ...
        3: Height
        4: Width
        This dictionary "onnx_coreml_shape_mapping" records onnxx shape to coreml shape mapping for
        every tensor (including intermediate tensors) in the onnxx graph. 
        The requirement is to only know the "rank" (i.e. number of dimensions) of the onnxx tensor, not its actual shape, during conversion time. 
        ...
        The dictionary "onnx_coreml_shape_mapping" is progressively built as the onnxx graph is converted to CoreML graph.
        ...
        # maps from onnx tensor name to the node that produces it
        self.output_map = {o: n for n in nodes for o in n.outputs} # type: Dict[Text, Node]

    def create_graph(self, nodes=None, inputs=None, outputs=None, initializers=None, shape_dict=None, onnx_ir_version=None):
        node = self.nodes if nodes is None else nodes
        inputs = self.inputs if inputs is None else inputs
        outputs = self.outputs if outputs is None else outputs
        initializers = self.initializers if initializers is None else initializers
        ...
        new_graph = Graph(nodes, inputs, outputs, initializers, shape_dict, onnx_ir_version)
        # Preserve the name map during graph creation copies
        new_graph.onnx_coreml_name_map = self.onnx_coreml_name_map
        return new_graph
    ...
    def from_onnx(graph, onnx_ir_version):  # type: (GraphProto) -> Graph
        ...
        return Graph(nodes_, inputs, outputs, input_tensors, shape_dict, onnx_ir_version)
