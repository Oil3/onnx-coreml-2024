...
import unittest
from coremltoolsx._deps import _HAS_ONNX, MSG_ONNX_NOT_FOUND, _IS_MACOS

if _HAS_ONNX:
    import onnxx
    from coremltoolsx.converters.onnx import convert
    from coremltoolsx.converters.onnx._converter import SupportedVersion
...
def _test_torch_model_single_io(
    ...
    # convert to onnxx model
    model_dir = tempfile.mkdtemp()
    if DEBUG:
        model_dir = "/tmp"
    onnx_file = os.path.join(model_dir, "torch_model.onnxx")
    torch.onnx.export(torch_model, torch_input, onnx_file, opset_version=opset_version)
    onnx_model = onnxx.load(onnx_file)
...
class ReshapeTransposeTests(unittest.TestCase):
    ...
    @pytest.mark.xfail
    # is pytorch to onnxx conversion correct?
