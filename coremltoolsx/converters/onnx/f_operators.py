...
def _get_coreml_target_shape(target_shape, builder, node, graph, err):
    ...
    if len(target_shape) == 1:  # (D,)
        ...
        print(target_shape)
        coreml_shape = (target_shape[0], target_shape[1], target_shape[2], target_shape[3], target_shape[4], target_shape[5]
        )
        graph.onnx_coreml_shape_mapping[node.outputs[0]] = [0,1,2,3,4,5]
#    elif len(target_shape) > 4:
#        print(target_shape)
#
