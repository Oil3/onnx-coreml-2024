...
def _add_conv_like_op(add_func, get_params_func, params_dict,
    ...
    rank = builder_get_rank(node.inputs[0])
    if rank == 4:
...
def _convert_constant_of_shape(builder, node, graph, err):
    ...
    """
    # Default to 0.0 if no "value" attribute is provided
    value = node.attrs.get("value", [0.0])

    # Ensure value is converted to a scalar
    if isinstance(value, (list, np.ndarray)) and len(value) > 0:
        value = float(value[0])  # Convert to scalar float for CoreML
    ...
    if node.inputs[0] in node.input_tensors:
        ...
        # Check if the output shape needs to be passed as unpacked integers
        builder.add_fill_static(
            name=node.name,
            output_name=node.outputs[0],
            output_shape=[int(dim) for dim in output_shape],  # Ensure unpacked integers
            value=value,  # Use the scalar value
        )
    else:
...
def _convert_gru(builder, node, graph, err):  # type: (NeuralNetworkBuilder, Node, Graph, ErrorHandling) -> None
    ...
    input_rank = builder_get_rank(node.inputs[0])
    if input_rank == -1:
...
def _convert_reshape(builder, node, graph, err):
    ...
    if shape_node in node.input_tensors:
        ...
        len_of_input_shape = builder_get_rank(node.inputs[0])
