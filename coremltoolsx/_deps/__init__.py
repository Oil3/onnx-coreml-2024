...
def _warn_if_above_max_supported_version(package_name, package_version, max_supported_version):
    if _get_version(package_version) > _StrictVersion(max_supported_version):
        _logging.warning(
            "%s version %s has not been tested with coremltoolsx. You may run into unexpected errors. "
...
try:
    ...
    if k_ver > _StrictVersion(_KERAS_MAX_VERSION):
        ...
        _logging.warning(
            (
                "Keras version %s has not been tested with coremltoolsx. You may run into unexpected errors. "
