...
import numpy as np
from coremltoolsx._deps import _HAS_TF, MSG_TF1_NOT_FOUND
...
import coremltoolsx
import coremltoolsx.models.datatypes as datatypes
from coremltoolsx.converters.mil.mil.ops.defs._utils import aggregated_pad
from coremltoolsx.models import _MLMODEL_FULL_PRECISION, _MLMODEL_HALF_PRECISION
from coremltoolsx.models import neural_network as neural_network
from coremltoolsx.models.neural_network import flexible_shape_utils
from coremltoolsx.models.utils import _macos_version, _is_macos
...
class CorrectnessTest(unittest.TestCase):
    ...
    def _compare_moments(model, inputs, expected, use_cpu_only=True, num_moments=10):
        ...
        if isinstance(model, str):
            model = coremltoolsx.models.MLModel(model)

        model = coremltoolsx.models.MLModel(model, useCPUOnly=use_cpu_only)
    ...
    def _test_model(
        ...
        elif isinstance(model, coremltoolsx.proto.Model_pb2.Model):
            model_dir = tempfile.mkdtemp()
            model_name = str(uuid.uuid4()) + ".mlmodel"
            model_path = os.path.join(model_dir, model_name)
            coremltoolsx.utils.save_spec(model, model_path)
            model = coremltoolsx.models.MLModel(model, useCPUOnly=useCPUOnly)

        # If we want to test the half precision case
        if model_precision == _MLMODEL_HALF_PRECISION:
            model = coremltoolsx.utils._convert_neural_network_weights_to_fp16(model)
...
class NewLayersSimpleTest(CorrectnessTest):
    ...
    def test_range_static_cpu(self, cpu_only=True):
        ...
        for param in params:
            ...
            coremltoolsx.utils.save_spec(builder.spec, model_path)
    ...
    def test_categorical_distribution_cpu_logits(self):
        ...
        for shape in shapes:
            ...
            model = coremltoolsx.models.MLModel(model, useCPUOnly=True)
...
def get_coreml_predictions_slice(X, params):
    ...
    try:
        ...
        output.name = "output"
        output.type.multiArrayType.dataType = coremltoolsx.proto.FeatureTypes_pb2.ArrayFeatureType.ArrayDataType.Value(
            "DOUBLE"
        )
        # save the model
        model_dir = tempfile.mkdtemp()
        ...
        coreml_model = coremltoolsx.models.MLModel(model_path)
        coreml_input = {"data": X}
