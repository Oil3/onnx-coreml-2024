...
import coremltoolsx
from coremltoolsx._deps import (
    _HAS_KERAS2_TF,
...
def create_model(spec):
    ...
    MLModel
    """
    return coremltoolsx.models.MLModel(spec)
...
class TestIODataTypes(unittest.TestCase):
    ...
    def _sklearn_setup(self, model, dtype, data, target):
        ...
        spec = coremltoolsx.converters.sklearn.convert(
            model, "data", "target"
        ).get_spec()
        return model, spec
    ...
    def test_keras_dense_model(self):
        ...
        for key, dtype in self.number_data_type.items():
            try:
                ...
                coreml_model = coremltoolsx.converters.keras.convert(
                    model, input_names=["data"], output_names=["target"]
                )
                spec = coreml_model.get_spec()
                spec.description.output[
                    0
                ].type.multiArrayType.dataType = coremltoolsx.proto.FeatureTypes_pb2.ArrayFeatureType.ArrayDataType.Value(
                ...
                coreml_model = coremltoolsx.models.MLModel(spec)
    ...
    def test_image_output_rgb(self):
        input_shape = (3, 10, 20)
        input_features = [("data", coremltoolsx.models.datatypes.Array(*input_shape))]
        output_features = [("target", coremltoolsx.models.datatypes.Array(*input_shape))]
        builder = coremltoolsx.models.neural_network.NeuralNetworkBuilder(
        ...
        output.type.imageType.colorSpace = coremltoolsx.proto.FeatureTypes_pb2.ImageFeatureType.ColorSpace.Value(
            "RGB"
        )
